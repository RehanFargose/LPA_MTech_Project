...
OUTPUT_DIR = f"My_Models/LegalBERT_Models/legalbert_fast_out_{start}-{end}_medium_{model_num}"
SEED = 42
...
MAX_WORDS = 1000        # keep first ~350 words (~512 tokens or fewer)
MAX_LENGTH = 256       # tokenizer max tokens
BATCH_SIZE = 2         
...
# ============================
# CONTINUE TRAINING: Epochs 4–6 (Fine-tuning Phase)
# Lower LR + add scheduler
# ============================

print("\nStarting fine-tuning phase (Epochs 4–6)...")
...
remaining_epochs = 3
total_steps_ft = len(train_loader) * remaining_epochs
warmup_ft = int(0.1 * total_steps_ft)

from transformers import get_linear_schedule_with_warmup
scheduler = get_linear_schedule_with_warmup(
    optimizer,
    num_warmup_steps=warmup_ft,
    num_training_steps=total_steps_ft
)

# Train for epochs 4–6
for epoch in range(3, 6):   # 0-based index → epoch4=3, epoch5=4, epoch6=5
    model.train()
    optimizer.zero_grad()
    epoch_loss = 0.0
    pbar = tqdm(train_loader, desc=f"Epoch {epoch+1}/6 (fine-tuning)", leave=False)
    ...
    # Evaluate after each fine-tune epoch
    val = evaluate(model, test_loader)
    print(f"\n[Fine-Tune] Epoch {epoch+1} finished. "
